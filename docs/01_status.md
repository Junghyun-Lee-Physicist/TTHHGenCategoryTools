...
| O2 | **analyzer loader 에 tree 이름 fallback 추가** — 다음 정리 작업 (2026-07-28 결정) | **현행 규약은 구규약 `ttnb_<KEY>.root` / tree `TtNb` 로 확정**했다 (2018 patch 도 `--out-tree TtNb` 로 뽑는다). 다음 업데이트: tempTTHH loader 가 `TtbarIdPatch` 를 먼저 보고 없으면 `TtNb` 로 **fallback**. 기본값을 그냥 뒤집으면 안 되는 이유 — 2017 patch 7편은 **파일 안의 tree 이름이 `TtNb`** 라 파일명 rename 으로 해결되지 않고 KNU Tier3 재추출이 필요하다. 근거·기각 이유는 [07](07_analyzer_integration.md) §4, 상대편 기록은 `tempTTHH/docs/STATUS.md` OPEN |
